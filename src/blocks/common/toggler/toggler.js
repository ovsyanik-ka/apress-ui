      ...
      $block
        ...
        .each(function () {
          var $this = $(this).data({
            ...
            $switcher = $this.find('.js-toggle-switcher').filter(function () {
              return (
                $(this).closest('.js-toggle').data('index') ==
                $this.data('index')
              );
            }),
            $content = $this.find('.js-toggle-content').filter(function () {
              return (
                $(this).closest('.js-toggle').data('index') ==
                $this.data('index')
              );
          ...
          $switcher.click(function (e) {
            ...
            switch ($block.data('effect')) {
              case 'slide':
                if (state) {
                  $content.show().slideUp(200, function () {
                    $win.resize();
                  });
                } else {
                  $content.hide().slideDown(200, function () {
                    $win.resize();
                  });
                }
                break;
              case 'fade':
                if (state) {
                  $content.show().fadeOut(200, function () {
                    $win.resize();
                  });
                } else {
