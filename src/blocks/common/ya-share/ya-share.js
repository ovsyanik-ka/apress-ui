...
app.modules.yandexShare = (function (self) {
  var _yaShareConfig = {
      theme: {
        lang: 'ru',
        services:
          (app.config.yaShare && app.config.yaShare.services) ||
  ...
  function _initYaShare() {
    $yaShareContainer.each(function () {
      ...
      app.config.yaShare[id] &&
        (_yaShareConfig.content = app.config.yaShare[id].content);
  ...
  self.load = function () {
    if ($yaShareContainer.length) {
      $.getScript('https://yastatic.net/share2/share.js').done(function () {
        _initYaShare();
      });
    }
  };
