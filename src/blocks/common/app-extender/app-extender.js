(function () {
  function _getConditionErrorMessagePairs(path, content) {
    return [
      [path[0] !== 'app', 'app object should be targeted.'],
      [!content, 'content object should be passed as second argument.'],
      [
  ...
  function _extendAppKey(pathToKey, content, isMergeSoft) {
    ...
    path.reduce(function (resultObj, currentScopeName, index) {
      if (!resultObj[currentScopeName]) {
        resultObj[currentScopeName] = {};
      }

      if (index === path.length - 1) {
        resultObj[currentScopeName] = Object.keys(content).reduce(function (
          result,
          currentKey
        ) {
          if (!result[currentKey] || !isMergeSoft) {
            result[currentKey] = content[currentKey];
          }
